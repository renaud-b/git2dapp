var GraphPublisher = {
    'dryRun': true,
    'tasks': [],
    'publish': function (userAddress, containerId, doneMessageId, continueButtonId, dryRun = true) {
        this.tasks = [];
        this.dryRun = dryRun;
        return new Promise((resolve, reject) => {
            var projectName = document.getElementById("project-name").value.trim();
            ProjectManager.projectName = projectName;
            ProjectManager.getProjectFiles(userAddress, projectName).then(function (projectFiles) {
                GraphPublisher._update(containerId, doneMessageId, continueButtonId, projectFiles, userAddress).then(resolve);
            }).catch(function () {
                GraphPublisher._create(containerId, doneMessageId, continueButtonId, userAddress).then(resolve);
            });
        });
    },
    '_update': function (containerId, doneMessageId, continueButtonId, projectFiles, userAddress) {
        const $this = this;
        return new Promise((resolve, reject) => {
            ...
            ProjectManager.files.forEach(function (file) {
                ...
                fileName.textContent = (file.file.path + (existing ? ((" (" + existing.id) + ")") : ((" (" + file.graphID) + ")")));
                row.appendChild(icon);
                row.appendChild(fileName);
                container.appendChild(row);
                var usedGraphID = (existing ? existing.id : file.graphID);
                console.log((("operation on file '" + file.file.path) + "' => "), (existing ? "update" : "create"), " with graphID: ", usedGraphID);
                var updatePromise = (existing ? GraphPublisher._updateFile(usedGraphID, file.link, file.file.path, existing, projectFiles, userAddress) : GraphPublisher._publishFile(usedGraphID, file.link, file.file.path, userAddress));
                $this.tasks.push({'file': file, 'existing': existing});
                updatePromise.then(function (state) {
                    icon.className = ((state === "AlreadyExists") ? "fas fa-equals text-gray-400" : "fas fa-check-circle text-green-600");
                    completed++;
                    if (completed === total) {
                        GraphPublisher._finalize(ProjectManager.files, userAddress, doneMessage, continueButton).then(resolve);
                    }
                }).catch(reject);
            });
        });
    },
    '_create': function (containerId, doneMessageId, continueButtonId, userAddress) {
        const $this = this;
        return new Promise((resolve, reject) => {
            var container = document.getElementById(containerId);
            var doneMessage = document.getElementById(doneMessageId);
    ...
    '_finalize': function (filesSource, userAddress, doneMessage, continueButton) {
        ...
        return new Promise((resolve, reject) => {
            var projectFiles = $this.tasks.map(function (task) {
                ...
                var type = null;
                if (name.endsWith(".html")) type = "text/html"; else if (name.endsWith(".css")) type = "text/css"; else if (name.endsWith(".js")) type = "text/javascript";
    ...
    '_publishFile': function (graphID, fileLink, fileName, userAddress) {
        return new Promise(function (resolve, reject) {
            FileProcessor.extractGraph(graphID, fileLink, fileName).then(function (b64Graph) {
                var payload = Blackhole.Actions.makeCreateGraph(graphID);
                GraphPublisher._writeTx(payload, userAddress).then(function () {
                    var payload = ((("urn:pi:graph:snap:" + graphID) + ":data:") + b64Graph);
                    console.log(("update the file " + fileName));
                    GraphPublisher._writeTx(payload, userAddress).then(resolve).catch(reject);
                }).catch(reject);
            }).catch(reject);
        });
    },
    '_updateFile': function (graphID, fileLink, fileName, existing, existingFiles, userAddress) {
        return new Promise(function (resolve, reject) {
            FileProcessor.extractGraph(graphID, fileLink, fileName, existingFiles).then(function (b64Graph) {
                Blackhole.getGraph(existing.id).then(function (g) {
                    var md5ExistingGraph = MD5(btoa(convertAccentsToHtmlCodes(JSON.stringify(g.object))));
    ...
    '_writeTx': function (payload, userAddress) {
        ...
        return new Promise(function (resolve, reject) {
            var event = {'type': "sign", 'recipient_blockchain_address': userAddress, 'data': payload, 'value': 0};
            if (eventManager) {
                eventManager.send(event, function (payload) {
                    Singularity.saveSignedTx(payload).then(function (tx) {
