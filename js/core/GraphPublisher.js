// Patch complet de GraphPublisher.js avec :
// 1. Utilisation correcte de existing.id lors d'un update
// 2. Création automatique des nouveaux fichiers lors d'un update
// 3. Correctif dans _finalize pour gérer .file.path ou .name

var GraphPublisher = {
    publish: function (userAddress, containerId, doneMessageId, continueButtonId) {
        ...
        ProjectManager.getProjectFiles(userAddress, projectName).then(function (projectFiles) {
            GraphPublisher._update(containerId, doneMessageId, continueButtonId, projectFiles, userAddress);
        }).catch(function () {
            GraphPublisher._create(containerId, doneMessageId, continueButtonId, userAddress);
        });
    },
    ...
    _update: function (containerId, doneMessageId, continueButtonId, projectFiles, userAddress) {
        ...
        ProjectManager.files.forEach(function (file) {
            ...
            var updatePromise = existing
                ? GraphPublisher._updateFile(usedGraphID, file.link, file.file.path, existing, projectFiles, userAddress)
                : GraphPublisher._publishFile(usedGraphID, file.link, file.file.path, userAddress);
