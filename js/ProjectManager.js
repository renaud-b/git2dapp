const ProjectManager = {
    'projectName': undefined, 'files': [], 'getFile': function (fileName) {
        ...
        const filter = JSON.stringify({
            ...
            'children': [{
                ...
                'value': address
            }, {'type': "has_prefix", 'field': "data", 'value': "urn:pi:tesseract:projects:"}]
        });
        return new Promise((resolve, reject) => {
            Singularity.filterTxAsync(filter).then((txList) => {
                if (txList.length == 0) {
                    resolve([]);
                    return;
                }
                ...
                for (let i = 0; (i < txList.length); i++) {
                    const parts = txList[i].data.replace("urn:pi:tesseract:projects:", "");
                    const projectName = parts.split(":")[0];
                    const rawProjectFiles = parts.split(":")[2];
                    try {
                        projects[projectName] = JSON.parse(atob(rawProjectFiles));
                    } catch (e) {
                        console.error((("Error parsing project files : '" + rawProjectFiles) + "'"), e);
                    }
                }
                resolve(projects);
            }).catch(reject);
        ...
        return new Promise((resolve, reject) => {
            Singularity.filterTxAsync(filter).then((txList) => {
                let files = [];
                if (txList.length === 0) {
                    reject(((("No file found for project " + projectName) + " of user ") + address));
                    return;
                }
